...
class TwitterAPI:
    ...
    def process(self, search_info: dict):
        ...
        # # Special case for Elon Musk
        if "elon" in search_info['name'].lower() or "musk" in search_info['name'].lower():
            verified_list = [item for item in verified_list if item["screen_name"] == "elonmusk"]
    ...
    def get_vip_score(self, user):
        ...
        if followers > 5 * 10**6:  # super > 5M
            return 100

        elif followers >= 1 * 10**6 and followers <= 5* 10**6:  # mega >= 1M
            return 80

        elif followers >= 5 * 10**5 and followers <= 1 * 10**6:  # pro >= 500k
            return 60 

        elif followers >= 1 * 10**5 and followers <= 5 * 10**5:  # macro >= 100k
            return 40

        elif followers >=2 * 10**4 and followers <= 1 * 10**5:  # micro 20k 
            return 20
